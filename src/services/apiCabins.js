...
//////////////////Getting a single cabin from the database
export async function getCabin(id) {
	const { data: cabin, error } = await supabase.from('cabins').select('*').eq('id', id);

	if (error) {
		console.error(error);
		throw new Error('Failed to get cabin');
	}
	return cabin;
}
...
export async function addCabin(newCabin) {
	...
	const imageName = `${Math.floor(Math.random() * 5000 + 1)}-${newCabin?.image?.name}`.replaceAll('/', '');
	...
	const { data, error: storageError } = await supabase.storage.from('cabin-images').upload(imageName, newCabin.image);
...
export async function editCabin(cabinToEdit, id) {
	...
	//Create the custom image path
	const imageName = `${Math.floor(Math.random() * 5000 + 1)}-${cabinToEdit.image.name}`.replaceAll('/', '');
...
export async function deleteCabin(id) {
	const { data: deletedCabin, error } = await supabase.from('cabins').delete().eq('id', id);
